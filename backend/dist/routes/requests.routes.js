...
const dueDate_1 = require("../utils/dueDate");
...
async function expireStaleAssignments() {
    await db_1.pool.query(`UPDATE requests r
     ...
     WHERE r.status = 'pending_accept'
       ...
       AND r.assigned_date < DATE_SUB(NOW(), INTERVAL ? HOUR)`, [AUTO_REJECT_HOURS]);
}
async function isFileUnavailable(fileId) {
    const [rows] = await db_1.pool.query(`SELECT 1 FROM requests
     WHERE file_id = ? AND status IN ('pending_accept','accepted')
     LIMIT 1`, [fileId]);
    return rows.length > 0;
}
router.get('/', auth_1.requireAuth, [
    (0, express_validator_1.query)('status').optional().isIn(['pending', 'pending_accept', 'accepted', 'returned', 'rejected_auto']),
    (0, express_validator_1.query)('mine').optional().isBoolean(),
...
router.post('/', auth_1.requireAuth, [
    ...
    (0, express_validator_1.body)('fileIds.*').optional().isInt({ min: 1 }),
    (0, express_validator_1.body)('urgentFileIds').optional().isArray(),
    (0, express_validator_1.body)('urgentFileIds.*').optional().isInt({ min: 1 }),
    (0, express_validator_1.body)('reasons').optional().isObject(),
    (0, express_validator_1.body)('confidentialFiles').optional().isArray(),
    (0, express_validator_1.body)('confidentialFiles.*.fileNumber').optional().trim().isLength({ min: 1, max: 64 }),
    (0, express_validator_1.body)('confidentialFiles.*.fileName').optional().trim().isLength({ min: 1, max: 255 }),
    (0, express_validator_1.body)().custom((v) => (v.fileIds?.length || 0) + (v.confidentialFiles?.length || 0) > 0)
        .withMessage('Provide at least one fileId or confidential file entry'),
], errorHandler_1.handleValidation, async (req, res, next) => {
    const conn = await db_1.pool.getConnection();
    try {
        const fileIds = req.body.fileIds || [];
        const urgentFileIds = new Set(req.body.urgentFileIds || []);
        ...
        const confidentialFiles = req.body.confidentialFiles || [];
        const created = [];
        const skipped = [];
        ...
        for (const cf of confidentialFiles) {
            ...
            await conn.query(`INSERT INTO registry_files (file_id, file_name, file_number, category)
           VALUES (?, ?, ?, 'confidential')
           ON DUPLICATE KEY UPDATE file_name = VALUES(file_name)`, [fileId, cf.fileName, cf.fileNumber]);
            const [rows] = await conn.query('SELECT id FROM registry_files WHERE file_id = ?', [fileId]);
        ...
        for (const fileId of fileIds) {
            ...
            const [result] = await conn.query(`INSERT INTO requests (file_id, requester_id, status, requested_date, is_urgent, reason)
           VALUES (?, ?, 'pending', NOW(), ?, ?)`, [fileId, req.user.sub, isUrgent ? 1 : 0, reason]);
            const requestId = result.insertId;
            await conn.query(`INSERT INTO movements (request_id, file_id, action, actor_user_id, subject_user_id, reason)
           VALUES (?, ?, 'pending', ?, ?, ?)`, [requestId, fileId, req.user.sub, req.user.sub, reason]);
...
router.post('/:id/request-again', auth_1.requireAuth, [(0, express_validator_1.param)('id').isInt({ min: 1 })], errorHandler_1.handleValidation, async (req, res, next) => {
    const conn = await db_1.pool.getConnection();
    try {
        ...
        await conn.beginTransaction();
        const [result] = await conn.query(`INSERT INTO requests (file_id, requester_id, status, requested_date)
         VALUES (?, ?, 'pending', NOW())`, [request.file_id, req.user.sub]);
        const newRequestId = result.insertId;
        await conn.query(`INSERT INTO movements (request_id, file_id, action, actor_user_id, subject_user_id)
         VALUES (?, ?, 'pending', ?, ?)`, [newRequestId, request.file_id, req.user.sub, req.user.sub]);
        await conn.commit();
...
router.post('/assign', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), [
    (0, express_validator_1.body)('fileId').isInt({ min: 1 }),
    (0, express_validator_1.body)('assignedToId').isInt({ min: 1 }),
    (0, express_validator_1.body)('requestId').optional().isInt({ min: 1 }),
    (0, express_validator_1.body)('registryCode').optional().trim().isLength({ max: 100 }),
    (0, express_validator_1.body)('actionFolio').optional().trim().isLength({ max: 100 }),
    ...
    try {
        ...
        const [userRows] = await conn.query('SELECT role FROM users WHERE id = ?', [assignedToId]);
        if (userRows.length === 0)
            return res.status(404).json({ error: 'Assignee not found' });
        const role = userRows[0].role;
        const dueDate = (0, dueDate_1.computeDueDate)(role, new Date());
        await conn.beginTransaction();
        let finalRequestId;
        if (requestId) {
            await conn.query(`UPDATE requests SET status = 'pending_accept', assigned_to_id = ?, assigned_date = NOW(),
             due_date = ?, registry_code = COALESCE(?, registry_code),
             action_folio = COALESCE(?, action_folio), reason = COALESCE(?, reason)
           WHERE id = ?`, [assignedToId, dueDate, registryCode || null, actionFolio || null, reason || null, requestId]);
            finalRequestId = requestId;
        }
        else {
            const [result] = await conn.query(`INSERT INTO requests (file_id, assigned_to_id, status, assigned_date, due_date, registry_code, action_folio, reason)
           VALUES (?, ?, 'pending_accept', NOW(), ?, ?, ?, ?)`, [fileId, assignedToId, dueDate, registryCode || null, actionFolio || null, reason || null]);
            finalRequestId = result.insertId;
        }
        await conn.query(`INSERT INTO movements (request_id, file_id, action, actor_user_id, subject_user_id, registry_code, action_folio, reason)
         VALUES (?, ?, 'pending_accept', ?, ?, ?, ?, ?)`, [finalRequestId, fileId, req.user.sub, assignedToId, registryCode || null, actionFolio || null, reason || null]);
        await conn.commit();
...
router.post('/:id/accept', auth_1.requireAuth, [(0, express_validator_1.param)('id').isInt({ min: 1 })], errorHandler_1.handleValidation, async (req, res, next) => {
    const conn = await db_1.pool.getConnection();
    try {
        const requestId = Number(req.params.id);
        const [rows] = await conn.query('SELECT * FROM requests WHERE id = ?', [requestId]);
        ...
        const [userRows] = await conn.query('SELECT role FROM users WHERE id = ?', [request.assigned_to_id]);
        const role = userRows[0]?.role || 'user';
        const dueDate = (0, dueDate_1.computeDueDate)(role, new Date());
        await conn.beginTransaction();
        await conn.query(`UPDATE requests SET status = 'accepted', accepted_date = NOW(), due_date = ? WHERE id = ?`, [dueDate, requestId]);
        await conn.query(`INSERT INTO movements (request_id, file_id, action, actor_user_id, subject_user_id)
         VALUES (?, ?, 'accepted', ?, ?)`, [requestId, request.file_id, req.user.sub, request.assigned_to_id]);
        await conn.commit();
...
router.post('/:id/return', auth_1.requireAuth, [
    (0, express_validator_1.param)('id').isInt({ min: 1 }),
    (0, express_validator_1.body)('actionFolio').optional().trim().isLength({ max: 100 }),
    (0, express_validator_1.body)('lastFolio').optional().trim().isLength({ max: 100 }),
    (0, express_validator_1.body)('reason').optional().trim().isLength({ max: 2000 }),
    (0, express_validator_1.body)('fileStatus').optional().isIn(['actioned', 'not_actioned', 'proceed_to']),
    (0, express_validator_1.body)('proceedToDest').optional().isIn([
        'chief_public_service', 'cs', 'dhrm', 'ddhrm', 'hro', 'payroll', 'fleet_manager',
    ]),
    (0, express_validator_1.body)('bringUpNote').optional().trim().isLength({ max: 2000 }),
], errorHandler_1.handleValidation, async (req, res, next) => {
    const conn = await db_1.pool.getConnection();
    try {
        const requestId = Number(req.params.id);
        ...
        if (!request)
            return res.status(404).json({ error: 'Request not found' });
        await conn.beginTransaction();
        await conn.query(`UPDATE requests SET status = 'returned', returned_date = NOW(), returned_by_id = ?,
        ...
        await conn.query(`INSERT INTO movements
...
router.post('/:id/forward', auth_1.requireAuth, [(0, express_validator_1.param)('id').isInt({ min: 1 }), (0, express_validator_1.body)('toUserId').isInt({ min: 1 })], errorHandler_1.handleValidation, async (req, res, next) => {
    try {
        const requestId = Number(req.params.id);
        const { toUserId } = req.body;
        const [rows] = await db_1.pool.query('SELECT * FROM requests WHERE id = ?', [requestId]);
        const request = rows[0];
        if (!request)
            return res.status(404).json({ error: 'Request not found' });
        if (request.status !== 'accepted') {
        ...
        const [targetRows] = await db_1.pool.query('SELECT id FROM users WHERE id = ? AND is_active = 1', [toUserId]);
        if (targetRows.length === 0)
            return res.status(404).json({ error: 'Selected user not found' });
        await db_1.pool.query('UPDATE requests SET assigned_to_id = ? WHERE id = ?', [toUserId, requestId]);
        res.json({ success: true });
    }
    catch (err) {
        next(err);
...
router.post('/:id/release', auth_1.requireAuth, [(0, express_validator_1.param)('id').isInt({ min: 1 })], errorHandler_1.handleValidation, async (req, res, next) => {
    const conn = await db_1.pool.getConnection();
    try {
        const requestId = Number(req.params.id);
        const [rows] = await conn.query('SELECT * FROM requests WHERE id = ?', [requestId]);
        ...
        await conn.query(`INSERT INTO movements (request_id, file_id, action, actor_user_id, subject_user_id)
         VALUES (?, ?, 'release', ?, ?)`, [requestId, request.file_id, req.user.sub, request.assigned_to_id]);
        await conn.commit();
        res.json({ success: true });
    }
    catch (err) {
