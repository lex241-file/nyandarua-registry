...
// eslint-disable-next-line @typescript-eslint/no-var-requires
const generalFiles = require('./data/general_files.json');
const ADMIN_USERS = [
...
async function upsertUser(conn, row, role) {
    const defaultPassword = row.id_number || row.file_number;
    const passwordHash = await bcryptjs_1.default.hash(String(defaultPassword), BCRYPT_COST);
    await conn.query(`INSERT INTO users (file_number, name, designation, id_number, role, password_hash, must_change_password, is_active)
     VALUES (?, ?, ?, ?, ?, ?, 1, 1)
     ON DUPLICATE KEY UPDATE name = VALUES(name), designation = VALUES(designation)`, [row.file_number, row.name, row.designation || '', row.id_number, role, passwordHash]);
...
async function main() {
    ...
    try {
        ...
        for (const p of personnel) {
            ...
            await conn.query(`INSERT INTO registry_files (file_id, file_name, file_number, category, owner_user_id)
         VALUES (?, ?, ?, 'personal', ?)
         ON DUPLICATE KEY UPDATE file_name = VALUES(file_name), owner_user_id = VALUES(owner_user_id)`, [fileId, p.name, p.file_number, userId]);
            count++;
            if (count % 500 === 0)
                console.log(`  ...${count}/${personnel.length} personnel imported`);
        }
        ...
        for (const g of generalFiles) {
         ...
         VALUES (?, ?, ?, 'general')
         ON DUPLICATE KEY UPDATE file_name = VALUES(file_name)`, [fileId, g.file_name, g.file_number]);
        }
