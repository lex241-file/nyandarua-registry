...
router.get('/directory', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const [rows] = await pool.query<any[]>(
      `SELECT id, file_number, name, role FROM users WHERE is_active = 1 ORDER BY name ASC`
    );
    res.json({ users: rows });
  } catch (err) {
    next(err);
