...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { fileNumber, name, designation = '', idNumber = null, role = 'user' } = req.body;
      ...
      await pool.query(
        `INSERT INTO users (file_number, name, designation, id_number, role, password_hash, must_change_password, is_active)
         VALUES (?, ?, ?, ?, ?, ?, 1, 1)`,
        [fileNumber, name, designation, idNumber, role, passwordHash]
      );
...
// Bulk edit (designation / role) — admin only.
router.patch(
  '/bulk',
  requireAuth,
  requireRole('admin'),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { fileNumbers, designation, role } = req.body as {
        fileNumbers: string[];
        designation?: string;
        role?: string;
      };
