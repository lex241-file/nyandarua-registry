...
router.get('/', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [[adminsRow]] = await pool.query<any[]>(
    ...
    const [[assignedRow]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS c FROM requests WHERE status IN ('pending_accept','accepted')"
    );
    const [[rejectedRow]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS c FROM requests WHERE status = 'rejected_auto'"
    );
...
router.get('/notifications', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.user!.role === 'admin') {
      const [[requestRow]] = await pool.query<any[]>(
        "SELECT COUNT(*) AS c FROM requests WHERE status = 'pending'"
      );
      const [[noteRow]] = await pool.query<any[]>(
        'SELECT COUNT(*) AS c FROM notes WHERE is_read = 0'
      );
