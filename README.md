...
mysql -u root -p < 08_migration_v7_forward_visibility.sql # forward/sign-for stay visible to both parties
mysql -u root -p < 03_app_user.sql    # creates a least-privilege app user (edit the password first!)
```

`02_migration_v2.sql` adds the full original-app feature set on top of the
base schema: the pending → pending_accept → accepted → returned lifecycle
with 12-hour auto-reject, Registry Code / Action Folio / Last Folio /
Reason / Proceed-To tracking, personal file sub-categories, and the
Confidential file category. It's safe to run even against an
already-seeded database — it only adds columns and migrates existing
status values, nothing is dropped. If you're on TiDB using its web SQL
Editor, run each statement in the file one at a time (same reason as
`01_schema.sql`'s trigger block — see below).
...
- **Audit log immutability**: the `movements` table can only be
  inserted into and read from — enforced two ways: (1) no update/delete
  route exists in the API, and (2) the app's database user has no
  UPDATE/DELETE grant on that table (`sql/03_app_user.sql`), so neither
  the application nor anyone using its same credentials via a raw SQL
  client can alter or erase a row. If you're on real MySQL/MariaDB
  (not TiDB), `sql/01_schema.sql` also includes an optional, commented-out
  trigger block for a third layer that protects the table even from a
  *different*, more privileged database connection — uncomment it if you
  want that extra layer. **TiDB Cloud does not support triggers at all**
  (confirmed in TiDB's own docs), so skip that block entirely if that's
  your database; the privilege-grant protection above still fully applies.
